...
describe Account, type: :model do
  ...
  let(:test_month) { Date.today.month }
  let(:test_year) { Date.today.year }
  ...
  describe 'Stripe Interactions' do
    ...
    describe 'Saving with stripe attributes' do
      ...
      context 'Valid attributes' do
        let(:token) do
          @token = get_token(name, cardnum, test_month, (test_year + 1),
                             cvcvalue)
        end
    ...
    describe 'Updating with stripe attributes' do
      ...
      context 'Valid stripe account update tests' do
        let(:first_token) do
          @first_token = get_token(name, cardnum, test_month, (test_year + 1),
                                   cvcvalue)
        end

        let(:second_token) do
          @second_token = get_token(new_name, cardnum, test_month, (test_year + 2),
                                    cvcvalue)
        end
      ...
      context 'Updating with invalid stripe attributes' do
        let(:third_token) do
          @third_token = get_token(name, cardnum, test_month, (test_year + 1),
                                   cvcvalue)
        end

        let(:forth_token) do
          @forth_token = get_token(new_name, cardnum, test_month, (test_year + 2),
                                   cvcvalue)
        end
    ...
    describe 'Get customer method' do
      ...
      context 'Valid customer get operation tests' do
        ...
        let(:info_token) do
          @info_token = get_token(name, cardnum, test_month, (test_year + 3),
                                  cvcvalue)
        end
        ...
        it 'Should retrieve the correct card expiration' do
          ...
          expect(user.account.expiration).to match(%r{#{month}/#{year}})
        end
      ...
      context 'Invalid get customer data tests' do
        ...
        let(:info_token) do
          get_token(name, cardnum, test_month, (test_year + 3),
                    cvcvalue)
        end
